...
if DEBUG then
  print ("[cantor-dust] origin: "..fractal_origin)
  print ("[cantor-dust] size: "..fractal_size)
end
...
function dust_test(d, x, y, z)
  ...
  -- else, return true
  if xOut or yOut or zOut then
    return false
  elseif d3 >= 3 then
    return dust_test(d3, x % d3, y % d3, z % d3)
  else
    return true
  end
end
...
minetest.register_on_generated(function(minp, maxp, seed)
  ...
  if outside_region(fractal_origin, fractal_size, minp, maxp) then
    if DEBUG then
      print("[cantor-dust] Skipping "..region_text(minp, maxp))
    end
  else
    if DEBUG then
      print ("[cantor-dust] Generating blocks in "..region_text(minp, maxp))
    end
    ...
    for z = minv.z, maxv.z do
      for y = minv.y, maxv.y do
        ...
        for x = minv.x, maxv.x do
          if dust_test(fractal_size, x - fractal_origin, y - fractal_origin, z - fractal_origin) then
